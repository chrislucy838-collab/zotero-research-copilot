...
只有配置的端点要求认证时才需要。未鉴权的本地端点可以将 API Key 留空。
...
API Key、设置、聊天记录、记忆和附件元数据保存在 Zotero 本地偏好、数据库和数据目录中。发起请求时，选中的上下文和附件会发送到配置的端点。

## 项目会收集遥测吗？

插件没有项目自建的遥测服务或代理。配置的端点仍会按照自己的政策处理收到的请求。
...
请提供 Zotero 版本、插件版本、API 模式、端点类型、模型 ID 和脱敏后的错误描述。不要上传 API Key、Cookie 或私人文档。可以在 [Issues](https://github.com/chrislucy838-collab/zotero-research-copilot/issues) 提交问题，或在 [Discussions](https://github.com/chrislucy838-collab/zotero-research-copilot/discussions) 讨论。
