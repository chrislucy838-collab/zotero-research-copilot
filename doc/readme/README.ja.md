...
**Tools → Add-ons → Zotero Research Copilot → Settings** を開き、API type で **OpenAI Compatible**、**Google Gemini**、**Anthropic Messages**、**OpenAI Responses**、または **ChatGPT Codex (Plus/Pro)** を選び、API Base URL と Model を入力します。API Key と Custom Headers はサービスが必要とする場合に設定します。例: `https://api.openai.com/v1`、`http://127.0.0.1:11434/v1`。通常は `/models` と `/chat/completions` に対応するエンドポイントが必要です。
