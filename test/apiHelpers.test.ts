...
describe("apiHelpers", function () {
  ...
  describe("buildHeaders", function () {
    ...
    it("should merge custom provider headers", function () {
      const headers = buildHeaders("sk-test", { "X-Project": "demo" });
      assert.equal(headers["X-Project"], "demo");
      assert.equal(headers.Authorization, "Bearer sk-test");
    });

    it("should preserve a custom Authorization header", function () {
      const headers = buildHeaders("sk-test", { Authorization: "Custom token" });
      assert.equal(headers.Authorization, "Custom token");
    });
  });
