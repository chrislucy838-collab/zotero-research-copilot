...
function authors(values: unknown): PaperAuthor[] {
  ...
  return values
    .map((value: any) => {
      const given = text(
        value?.author?.given ?? value?.given ?? value?.firstName,
      );
      const family = text(
        value?.author?.family ?? value?.family ?? value?.lastName,
      );
      const display =
        text(value?.author?.display_name ?? value?.name ?? value?.authorName) ||
...
export async function searchOpenAlex(
  ...
  return (Array.isArray(payload?.results) ? payload.results : []).map(
    (row: JsonObject) => {
      ...
      return {
        ...
        pdfSource: safePdfUrl(
          primaryLocation.pdf_url ?? row.open_access?.oa_url,
        )
          ? "openalex"
          : undefined,
        citationCount: Number(row.cited_by_count) || undefined,
        openAccess: row.open_access?.is_oa === true,
        pdfStatus: safePdfUrl(
          primaryLocation.pdf_url ?? row.open_access?.oa_url,
        )
          ? "available"
          : "unknown",
...
export async function searchSemanticScholar(
  ...
  const fields =
    "title,abstract,authors,year,venue,externalIds,url,openAccessPdf,citationCount";
  ...
  const payload = await fetchJson(url, signal);
  return (Array.isArray(payload?.data) ? payload.data : []).map(
    (row: JsonObject) =>
      ({
        source: "semanticScholar",
        sourceId:
          text(row.paperId) ||
          text(row.externalIds?.DOI) ||
          "semantic-scholar-unknown",
        title: text(row.title) || "Untitled paper",
        ...
        pdfSource: safePdfUrl(row.openAccessPdf?.url)
          ? "semanticScholar"
          : undefined,
...
function extractArxivId(value: string): string | undefined {
  const match = value.match(
    /(?:arxiv\.org\/(?:abs|pdf)\/|arxiv\s*:\s*)?(\d{4}\.\d{4,5})(?:v\d+)?/i,
  );
  return match?.[1];
}
...
export async function fetchArxivPaper(
  ...
  return entries.map((entry) => {
    const id =
      extractArxivId(extractXmlTag(entry, "id") || normalizedId) ||
      normalizedId;
    const authorNames = Array.from(
      ...
      name: text(match[1]),
      lastName: (text(match[1]) || "").split(/\s+/).pop() || "",
    }));
...
function crossrefCandidate(row: JsonObject): PaperCandidate {
  return {
    source: "crossref",
    sourceId: doi(row.DOI) || text(row.URL) || "crossref-unknown",
    title:
      text(Array.isArray(row.title) ? row.title[0] : row.title) ||
      "Untitled paper",
    authors: authors(row.author),
    year: year(
      row.published?.["date-parts"]?.[0]?.[0] ??
        row.publishedPrint?.["date-parts"]?.[0]?.[0],
    ),
...
export async function searchCrossref(
  ...
  ).map((row: JsonObject) => crossrefCandidate(row));
}
...
export async function lookupCrossrefByDoi(
  ...
  try {
    const payload = (await fetchJson(url, signal)) as JsonObject;
    return payload?.message ? crossrefCandidate(payload.message) : null;
  } catch {
    return null;
  }
}

export const providerSearchers: Record<
  PaperSource,
  (
    query: string,
    limit: number,
