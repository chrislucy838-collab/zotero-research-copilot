...
export async function bootstrapSettingTab(
  ...
  // Internal placeholder retained for shared refresh code; never shown.
  ...
  oauthTabBtn.style.display = "none";
  ...
  modeTabBar.append(customTabBtn);

  // CPA is the only supported connection mode in this fork.
  ...
  connectionModeBody.append(customModeRadio, customPanel);
  ...
  fetchModelsBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      customModelHint.textContent = L.fetchModelsFailed;
      customModelHint.style.color = "#dc2626";
      ztoolkit?.log?.("Zotero Research Copilot: CPA model fetch failed", err);
    } finally {
  ...
  // OAuth provider cards are intentionally not part of this CPA-only fork.
  // Keep the legacy nodes detached so shared cleanup code remains harmless.
  ...
  // Translation is handled by Zotero PDF Translate and is not rendered here.
