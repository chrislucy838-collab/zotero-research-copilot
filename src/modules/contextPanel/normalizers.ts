...
export function normalizePaperContextRefs(
  ...
  for (const entry of value) {
    ...
    const sourceId = normalizeText(typed.sourceId, sanitize);
    const sourceLabel = normalizeText(typed.sourceLabel, sanitize);
    const sourceKind =
      typed.sourceKind === "active-document" ||
      typed.sourceKind === "supplemental-paper"
        ? typed.sourceKind
        : undefined;
    const availability =
      typed.availability === "full-text" ||
      typed.availability === "metadata-only" ||
