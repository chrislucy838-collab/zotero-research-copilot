...
import { createEvidenceBlock, type EvidenceBlock } from "../evidence";
...
export type BuildDocumentContextOptions = {
  ...
  /** Receives the exact chunks emitted into the returned context. */
  onEvidenceBlocks?: (blocks: EvidenceBlock[]) => void;
  /** Identity used by EvidenceBlock when this context belongs to a paper. */
  evidenceSource?: {
    sourceId?: string;
    sourceLabel?: string;
    itemId?: number;
  ...
  const prefix =
    metadata.role === "notes"
      ...
      : metadata.role === "frontmatter"
        ...
        : `${nounLabel} section`;
  const page =
    metadata.locator?.kind === "pdf-page"
      ? metadata.locator.pageLabel || `p. ${metadata.locator.pageIndex + 1}`
      : "";
  const location = page ? ` · ${page}` : "";
  return heading ? `${prefix} — ${heading}${location}` : `${prefix}${location}`;
}
...
export async function buildDocumentContext(
  ...
  if (FORCE_FULL_CONTEXT && contextStrategy === "full-or-retrieval") {
    if (!fullLength || fullLength <= FULL_CONTEXT_CHAR_LIMIT) {
      ...
      contextParts.push(
        chunks
          .map((chunk, index) => `${getChunkLabel(context, index)}:\n${chunk}`)
          .join("\n\n"),
      );
      options?.onEvidenceBlocks?.(
        chunks.map((chunk, index) =>
          createEvidenceBlock({
            ...options.evidenceSource,
            quote: chunk,
            metadata: context.chunkMetadata?.[index],
            chunkIndex: index,
          }),
        ),
      );
  ...
  if (excerpts.length) {
    const evidenceBlocks = emittedIndexes.map((index) =>
      createEvidenceBlock({
        ...(options?.evidenceSource || {}),
        quote: chunks[index] || "",
        metadata: structuredMetadata?.[index],
        chunkIndex: index,
        score: scored.find((entry) => entry.index === index)?.score,
      }),
    );
    ...
    contextParts.push(
      excerpts
        .map((excerpt, index) =>
          evidenceBlocks[index]
            ? `[evidence:${evidenceBlocks[index].evidenceId}]\n${excerpt}`
            : excerpt,
        )
        .join("\n\n"),
    );
    options?.onEvidenceBlocks?.(evidenceBlocks);
  } else {
    options?.onEvidenceBlocks?.([]);
  }
