...
import {
  ...
  CONTEXT_COMPACTION_SUMMARY_INPUT_TOKENS,
} from "./constants";
import type {
  Message,
  AdvancedModelParams,
...
import { formatEvidenceInstruction } from "./evidence";
import {
...
function normalizePaperContexts(paperContexts: unknown): PaperContextRef[] {
  return normalizePaperSourceRefs(
    normalizePaperContextRefs(paperContexts, { sanitizeText }),
    "supplemental-paper",
  );
}
...
async function openEvidencePage(evidence: EvidenceBlock | EvidenceBlock[]): Promise<void> {
  ...
  try {
    ...
    ztoolkit.log("LLM: Failed to open evidence page", err);
    Zotero.alert(
      Zotero.getMainWindow(),
      "Zotero Research Copilot",
      "Unable to open the cited page in Zotero Reader.",
    );
  }
}
...
export async function copyTextToClipboard(
  ...
  if (!safeText) throw new Error("No text available to copy");
  ...
  if (typeof internalCopy === "function") {
    try {
      internalCopy(safeText);
      return;
    } catch (err) {
      ztoolkit.log("Zotero internal clipboard copy failed:", err);
    }
  }
  ...
  throw new Error("Zotero clipboard service is unavailable");
}
...
export async function exportMarkdownToFile(
  body: Element,
  markdownText: string,
  suggestedName = "zotero-research-copilot-chat.md",
): Promise<string | null> {
  ...
  const selectedPath = await new FilePickerHelper(
  ...
  await zotero.File.putContentsAsync(path, `${safeText}\n`);
  return path;
}
...
export function resolveEffectiveRequestConfig(params: {
  ...
  const modelFallback = getStringPref("model");
  const model = (
    params.model ||
...
async function buildCombinedContextForRequest(params: {
  ...
  for (const entry of pool.supplementalContexts.values()) {
    requestEvidenceBlocks.push(...(entry.evidenceBlocks || []));
  }
        ...
        pdfContext = await buildReaderDocumentContext(
          ...
          { apiBase: params.apiBase, apiKey: params.apiKey },
          {
            ...
            onRetrievedSegments: queryDependent
              ...
              : undefined,
            onEvidenceBlocks: (blocks) => {
              pool.baseEvidenceBlocks = blocks;
              requestEvidenceBlocks.push(...blocks);
              params.onEvidenceBlocks?.(blocks);
            },
            evidenceSource: {
              sourceId: getPaperSourceId({
                itemId: pool.basePdfItemId,
      ...
      pdfContext = readerDocument
        ? await buildReaderDocumentContext(
            ...
            { apiBase: params.apiBase, apiKey: params.apiKey },
            {
              ...
              evidenceSource: {
                sourceId: getPaperSourceId({
                  itemId: ctxItem.parentID || ctxItem.id,
                  contextItemId: ctxItem.id,
                }),
                sourceLabel: "Paper 1",
                itemId: ctxItem.parentID || ctxItem.id,
                contextItemId: ctxItem.id,
  ...
  const hasActivePaperSource =
    pool.basePdfItemId !== null && !pool.basePdfRemoved;
  const rawPaperRefs = relabelPaperSourceRefs(
    params.paperContexts,
    hasActivePaperSource ? 1 : 0,
    "supplemental-paper",
  );
  ...
  for (const ref of currentPaperRefs) {
    ...
    if (existing && existing.builtContext) {
      requestEvidenceBlocks.push(...(existing.evidenceBlocks || []));
      continue; // Already built, skip.
    }
    const sourceRef = normalizePaperSourceRef(
      ref,
      pool.supplementalContexts.size +
        (pool.basePdfItemId !== null && !pool.basePdfRemoved ? 1 : 0),
      "supplemental-paper",
    );
  ...
  const supplementalBlocks = [...pool.supplementalContexts.values()]
    ...
    .filter(Boolean);
  const boundedSupplementalBlocks: string[] = [];
  let supplementalRemaining = SUPPLEMENTAL_PAPER_CONTEXT_TOTAL_MAX_LENGTH;
  for (const block of supplementalBlocks) {
    if (supplementalRemaining <= 0) break;
    if (block.length <= supplementalRemaining) {
      boundedSupplementalBlocks.push(block);
  ...
  if (pdfContext.trim() && pool.basePdfItemId !== null && !pool.basePdfRemoved) {
    ...
    try {
      ...
      if (baseAttachment?.parentID) baseSourceItemId = baseAttachment.parentID;
    } catch (_e) {
      // Keep the attachment ID as a stable fallback when Zotero is unavailable.
    }
    const baseSource = {
      itemId: baseSourceItemId,
      contextItemId: pool.basePdfItemId,
      ...
      sourceLabel: getPaperSourceLabel(0),
      availability: "full-text" as const,
    };
    pdfContext = `${formatPaperSourceHeader(baseSource)}\n[${baseSource.sourceLabel}] document text begins\n\n${pdfContext.trim()}\n[/${baseSource.sourceLabel}]`;
  }
  ...
  const assembledContext = [
    ...
    .filter(Boolean)
    .join("\n\n====================\n\n");
  const contextTokens = estimateTextTokens(assembledContext);
  if (contextTokens > getContextCompactionTriggerTokens()) {
    ztoolkit.log(
      `LLM context: assembled context ${contextTokens} tokens exceeds ${getContextCompactionTriggerTokens()}-token pre-compaction budget for ${getContextWindowTokens()}-token window; applying head/tail safety bound`,
    );
    return truncateTextHeadTail(
      assembledContext,
      getContextCompactionTriggerTokens(),
      "\n\n[Earlier document/paper context truncated before the active context window. Source labels and the latest evidence remain below.]\n\n",
    );
  }
...
function buildContextRefsSnapshot(
  ...
  if (pool.basePdfItemId !== null) {
    ...
    const baseDocument = {
      ...
      sourceLabel: getPaperSourceLabel(0),
      availability: pool.basePdfContext
        ? ("full-text" as const)
  ...
  if (pool.supplementalContexts.size > 0) {
    refs.supplementalPapers = [...pool.supplementalContexts.values()].map(
      (entry, index) =>
        normalizePaperSourceRef(
          entry.ref,
          index + (pool.basePdfItemId !== null && !pool.basePdfRemoved ? 1 : 0),
          "supplemental-paper",
        ),
    );
  }
  const persistedEvidenceBlocks = [
    ...(pool.baseEvidenceBlocks || []),
    ...[...pool.supplementalContexts.values()].flatMap(
      (entry) => entry.evidenceBlocks || [],
    ),
  ];
  ...
  if (cachedZoneBSummary) {
    ...
    const boundary = zoneBSummaryBoundaryCache.get(conversationKey);
    if (boundary) refs.compactionBoundary = boundary;
  }
...
function restoreContextPoolFromStoredMessages(
  ...
  if (!latestContextRefs) {
    if (options?.force) {
      ...
      zoneBSummaryBoundaryCache.delete(conversationKey);
    }
    return;
  }
  ...
  const pool: ConversationContextPoolEntry = {
    ...
    baseDocumentSegmentIds: Array.isArray(baseDocumentRef?.retrievalSegmentIds)
      ...
      : [],
    baseEvidenceBlocks: Array.isArray(latestContextRefs.evidenceBlocks)
      ? latestContextRefs.evidenceBlocks
      : [],
    supplementalContexts: new Map(),
  };
  ...
  if (
    ...
    if (typeof latestContextRefs.compactionBoundary === "string") {
      zoneBSummaryBoundaryCache.set(
        conversationKey,
        latestContextRefs.compactionBoundary,
      );
    }
    ztoolkit.log(
...
/** Last message incorporated into each summary; enables incremental compaction. */
export const zoneBSummaryBoundaryCache = new Map<number, string>();
...
function estimateHistoryLength(messages: Message[]): number {
  return messages.reduce((total, message) => {
    total += stripGeneratedImageMarkdown(message.text).length;
    if (message.selectedText) total += message.selectedText.length;
    if (Array.isArray(message.selectedTexts)) {
      for (const text of message.selectedTexts) total += (text || "").length;
    }
    return total;
  }, 0);
}
...
function selectHistoryWithinTokenBudget(
  history: Message[],
  budgetTokens = EFFECTIVE_CONTEXT_WINDOW_TOKENS,
): Message[] {
  const selected: Message[] = [];
  let used = 0;
  for (let index = history.length - 1; index >= 0; index -= 1) {
    const message = history[index];
    if (!isUsableLLMHistoryMessage(message)) continue;
    const cost = Math.max(1, estimateHistoryTokens([message]));
...
function hardTruncateHistory(
  history: Message[],
  keepTokens = CONTEXT_HARD_TRUNCATION_KEEP_TOKENS,
): Message[] {
  const kept = selectHistoryWithinTokenBudget(history, keepTokens);
  if (kept.length && kept.length < history.length) {
    const marker: Message = {
      role: "assistant",
      text: "[Earlier conversation history was truncated to keep the session usable.]",
      timestamp: Date.now(),
    };
    return [marker, ...kept];
  }
...
const COMPACTION_SUMMARY_PROMPT =
  `Summarise the following conversation history into an incremental checkpoint. ` +
  `Merge it with the previous checkpoint when one is provided.\n\n` +
  `Return exactly this structure:\n` +
  `## Goal\n[What is the user trying to accomplish?]\n\n` +
  `## Constraints & Preferences\n[List important requirements and preferences, or (none).]\n\n` +
  `## Progress\n### Done\n- [Completed work]\n\n### In Progress\n- [Current work]\n\n### Blocked\n- [Blockers, or (none).]\n\n` +
  `## Key Decisions\n- [Decision and rationale]\n\n` +
  `## Next Steps\n1. [Next action]\n\n` +
  `## Critical Context\n- [Facts, names, paths, evidence, errors, and source boundaries needed to continue.]\n\n` +
  `Keep the checkpoint concise but preserve concrete names and unresolved questions. Write in the same language as the conversation.\n\n` +
...
async function compactConversationHistory(params: {
  ...
  const shouldCompact = estimatedTokens >= getContextCompactionTriggerTokens();
  ...
  if (!shouldCompact && !cachedSummary) {
    return buildLLMHistoryMessages(
      selectHistoryWithinTokenBudget(usableHistory, historyBudget),
    );
  }
  if (!shouldCompact && cachedSummary) {
  ...
  const summarySafeHistory = selectHistoryWithinTokenBudget(
    incrementalHistory,
    CONTEXT_COMPACTION_SUMMARY_INPUT_TOKENS,
  );
  if (summarySafeHistory.length < usableHistory.length) {
    summarySafeHistory.unshift({
      role: "assistant",
      text: "[The oldest conversation history was omitted from this compaction pass because the summary request has its own safety budget.]",
      timestamp: Date.now(),
    });
  }
  const { zoneBMessages, zoneCMessages } = buildZoneBCSplit(summarySafeHistory);
  ...
  if (zoneBMessages.length > 0) {
    ...
    try {
      ...
      if (summary && summary.trim().length > 20) {
        ...
        const newestCompacted = zoneBMessages.at(-1);
        if (newestCompacted) {
          zoneBSummaryBoundaryCache.set(
            params.conversationKey,
            messageBudgetKey(newestCompacted),
          );
        }
        ztoolkit.log(
  ...
  result.push(
    ...buildLLMHistoryMessages(
      selectHistoryWithinTokenBudget(zoneCMessages, historyBudget),
    ),
  );
  return result;
}
...
export async function editUserMessageAndRetry(
  ...
  const historyForLLM = history.slice(0, sourceUserIndex);
...
export function refreshConversationIndex(body: Element): void {
  ...
  for (const wrapper of userWrappers) {
    ...
    button.type = "button";
    button.className = "llm-conversation-index-item";
    button.dataset.messageId = messageId;
    button.textContent = getConversationIndexTitle(bubble.textContent || "");
    button.title = bubble.textContent || button.textContent;
  ...
  // Keep the visible window on the newest questions while retaining every
  // older entry in the scrollable index. Once the user has scrolled upward,
  // preserve that position across chat redraws.
  if (!hadEntries || wasAtBottom) {
  ...
  if (!userWrappers.length) {
    empty.className = "llm-conversation-index-empty";
  ...
  if (!item) {
    chatBox.innerHTML = `
        <div class="llm-welcome">
          <div class="llm-welcome-icon">ZRC</div>
        <div class="llm-welcome-text">${i18n.statusSelectItem}</div>
        </div>
      `;
    refreshConversationIndex(body);
    return;
  }
    ...
    if (isUser) {
      ...
      if (paperContexts.length) {
        ...
        for (const [paperIndex, paperContext] of paperContexts.entries()) {
          ...
          const rowLabel =
            paperContext.sourceLabel || getPaperSourceLabel(displayIndex - 1);
          const metaParts = [
