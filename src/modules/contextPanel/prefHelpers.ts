...
export type PrimaryConnectionMode = "custom";
...
export function migratePrimaryConnectionMode(): PrimaryConnectionMode {
  // This fork uses one connection path: a user-configured OpenAI-compatible provider.
  Zotero.Prefs.set(
...
export function getPrimaryConnectionMode(): PrimaryConnectionMode {
  if (getStringPref(PRIMARY_CONNECTION_MODE_PREF_KEY) !== "custom") {
...
export function getSelectedProfileForItem(itemId: number): {
  ...
  const model =
    cachedValue && !MODEL_PROFILE_KEYS.has(cachedValue as ModelProfileKey)
      ? cachedValue
      : persistedModelName || profiles.primary.model;
  return {
    key: "primary",
