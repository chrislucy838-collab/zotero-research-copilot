...
import {
  ...
  estimateTextTokens,
  getContextCompactionTriggerTokens,
...
import { bootstrapProviderSettings } from "../cpaSettings";
...
export function setupHandlers(body: Element, initialItem?: Zotero.Item | null) {
  ...
  if (
    settingScroll &&
    settingScroll.dataset.renderVersion !== "provider-settings-v1"
  ) {
    settingScroll.dataset.renderVersion = "provider-settings-v1";
    void bootstrapProviderSettings(panelDoc, settingScroll).catch((error) => {
      ztoolkit.log("Zotero Research Copilot: provider settings failed", error);
    });
  }
  const discoverScroll = panelRoot.querySelector(
    "#llm-discover-scroll",
  ) as HTMLElement | null;
  if (
    discoverScroll &&
    panelRoot.dataset.activeTab === "discover" &&
    !discoverScroll.dataset.discoveryState
  ) {
    discoverScroll.dataset.discoveryState = "loading";
    void bootstrapPaperDiscovery(
      ...
      .catch((error) => {
        discoverScroll.dataset.discoveryState = "error";
        const failure = panelDoc.createElement("div");
        ...
        discoverScroll.replaceChildren(failure);
        ztoolkit.log(
  ...
  const shieldInputEventFromZoteroShortcuts = (event: Event) => {
    ...
    // keep its default editing behavior, but keep those events inside Zotero Research Copilot.
  ...
  if (contentWrapper && bottomWrapper) {
    ...
    if (panelRoot.dataset.activeTab === "discussion") {
      heightSync.switchToDiscussion();
    } else if (panelRoot.dataset.activeTab === "setting") {
      heightSync.switchToSetting();
    }
    (
      panelRoot as typeof panelRoot & {
  ...
  const getTextContextConversationKey = (): number | null =>
    Number.isFinite(conversationKey) && (conversationKey as number) > 0
      ...
      : null;
  const updateContextUsageIndicator = () => {
    if (!contextUsage || !contextUsageRing) return;
    const windowTokens = getContextWindowTokens();
    const usageSlot = contextUsage.parentElement as HTMLElement | null;
    if (usageSlot) usageSlot.style.display = "inline-flex";
    const key = item ? getConversationKey(item) : null;
    const history = key ? chatHistory.get(key) || [] : [];
    const papers = item
      ? normalizePaperContextEntries(
    ...
    const paperText = papers
      .map((paper) =>
        [paper.title, paper.firstCreator, paper.year].filter(Boolean).join(" "),
      )
      .join("\n");
    const inputTokens = estimateTextTokens(inputBox?.value || "");
    const paperTokens = estimateTextTokens(paperText);
    let historyTokens = estimateHistoryTokens(history);
    // A panel can render a just-loaded conversation one tick after the handler
    // is initialized. Use the visible bubbles as a safe fallback so the meter
    // never reports 0 while the user can visibly see conversation content.
    if (historyTokens === 0 && chatBox) {
      const renderedConversationText = Array.from(
        chatBox.querySelectorAll(".llm-bubble"),
      )
        .map((bubble) => (bubble as HTMLElement).textContent || "")
        .join("\n");
      historyTokens = estimateTextTokens(renderedConversationText);
    }
    const usedTokens = historyTokens + inputTokens + paperTokens;
    ...
    contextUsageRing.style.setProperty(
      "--llm-context-usage",
      `${compactPercent}`,
    );
    const usedLabel =
    ...
    const windowLabel =
      windowTokens >= 1000000
        ? `${(windowTokens / 1000000).toFixed(1)}M`
        : `${Math.round(windowTokens / 1000)}K`;
    const triggerLabel =
      getContextCompactionTriggerTokens() >= 1000000
        ? `${(getContextCompactionTriggerTokens() / 1000000).toFixed(1)}M`
        : `${Math.round(getContextCompactionTriggerTokens() / 1000)}K`;
    const label = `Context estimate: ${usedLabel} / ${windowLabel} tokens (${percent.toFixed(1)}%). Auto-compaction near ${triggerLabel}.`;
    contextUsage.title = label;
  ...
  if (conversationIndex && conversationIndexToggle && conversationIndexList) {
    const indexState = { collapsed: false };
    const applyConversationIndexLayout = () => {
      if (indexState.collapsed) {
        conversationIndex.style.setProperty("height", "22px", "important");
        conversationIndex.style.setProperty("min-height", "22px", "important");
        conversationIndex.style.setProperty("max-height", "22px", "important");
        conversationIndexList.style.setProperty("display", "none", "important");
      } else {
        conversationIndex.style.setProperty("height", "160px", "important");
        conversationIndex.style.setProperty("min-height", "160px", "important");
        conversationIndex.style.setProperty("max-height", "160px", "important");
        conversationIndexList.style.removeProperty("display");
        conversationIndexList.style.setProperty("position", "absolute");
        conversationIndexList.style.setProperty("inset", "26px 0 0 0");
        conversationIndexList.style.setProperty("height", "134px", "important");
        conversationIndexList.style.setProperty(
          "min-height",
          "134px",
          "important",
        );
    ...
    conversationIndexToggle.addEventListener("click", (event: Event) => {
      event.preventDefault();
      event.stopPropagation();
      indexState.collapsed = !indexState.collapsed;
      conversationIndex.classList.toggle("collapsed", indexState.collapsed);
      conversationIndexToggle.dataset.collapsed = indexState.collapsed
        ? "true"
        : "false";
      conversationIndexToggle.classList.toggle(
        "llm-conversation-index-toggle-collapsed",
        indexState.collapsed,
      );
      conversationIndexToggle.title = indexState.collapsed
        ? "Expand conversation index"
        : "Collapse conversation index";
      conversationIndexToggle.setAttribute(
        "aria-label",
        conversationIndexToggle.title,
      );
      applyConversationIndexLayout();
    });
    applyConversationIndexLayout();

    const chatBoxForIndex = body.querySelector(
      "#llm-chat-box",
    ) as HTMLElement | null;
    const updateActiveIndexItem = () => {
      if (!chatBoxForIndex) return;
      const chatRect = chatBoxForIndex.getBoundingClientRect();
      let activeId = "";
      let bestDistance = Number.POSITIVE_INFINITY;
      for (const wrapper of Array.from(
        chatBoxForIndex.querySelectorAll(
          ".llm-message-wrapper.user[data-message-id]",
        ),
      ) as HTMLElement[]) {
        const rect = wrapper.getBoundingClientRect();
        const distance = Math.abs(rect.top - chatRect.top - 18);
        if (rect.top <= chatRect.bottom && distance < bestDistance) {
          bestDistance = distance;
          activeId = wrapper.dataset.messageId || "";
        }
      }
      for (const button of Array.from(
        conversationIndexList.querySelectorAll(".llm-conversation-index-item"),
      ) as HTMLButtonElement[]) {
        button.classList.toggle(
          "active",
          Boolean(activeId && button.dataset.messageId === activeId),
        );
      }
    };
    chatBoxForIndex?.addEventListener("scroll", updateActiveIndexItem, {
      passive: true,
    });
    conversationIndexList.addEventListener(
      "wheel",
      (event: WheelEvent) => {
        const canScrollUp = conversationIndexList.scrollTop > 0;
        const canScrollDown =
          conversationIndexList.scrollTop + conversationIndexList.clientHeight <
          conversationIndexList.scrollHeight;
        const movingUp = event.deltaY < 0;
        const movingDown = event.deltaY > 0;
        if ((movingUp && canScrollUp) || (movingDown && canScrollDown)) {
          event.stopPropagation();
        }
      },
      { passive: true },
  ...
  if (exportMenu && exportMenuCopyBtn && exportMenuNoteBtn) {
    if (!exportMenu.dataset.listenerAttached) {
      ...
      exportMenuCopyBtn.addEventListener("click", async (e: Event) => {
        ...
        if (!item) {
          if (status)
            setStatus(status, getPanelI18n().noChatHistoryDetected, "error");
          return;
        }
        try {
          ...
          ztoolkit.log("Copy chat markdown failed:", err);
          if (status) {
            const message = err instanceof Error ? err.message : String(err);
            setStatus(status, getPanelI18n().operationFailed(message), "error");
          }
        }
      });
      exportMenuFileBtn?.addEventListener("click", async (e: Event) => {
        e.preventDefault();
        e.stopPropagation();
        closeExportMenu();
        if (!item) {
          if (status)
            setStatus(
              status,
              getPanelI18n().operationFailed("No active Zotero item"),
              "error",
            );
          return;
        }
        try {
          ...
          const title =
            normalizeHistoryTitle(item.getField?.("title")) || "chat";
          const savedPath = await exportMarkdownToFile(
            body,
            payload.noteText,
            `${title}.md`,
          );
          if (savedPath && status)
            setStatus(status, `Markdown exported: ${savedPath}`, "ready");
        } catch (err) {
          ztoolkit.log("Export chat markdown file failed:", err);
          if (status) {
            const message = err instanceof Error ? err.message : String(err);
            setStatus(status, getPanelI18n().operationFailed(message), "error");
          }
      ...
      exportMenuNoteBtn.addEventListener("click", async (e: Event) => {
        ...
        try {
          ...
          if (status)
            setStatus(
              status,
              `${getPanelI18n().savedChatHistoryToNewNote} (Note #${noteId})`,
              "ready",
            );
        } catch (err) {
  ...
  // refreshChat() is also called by the panel bootstrap modules after this
  // handler is installed. Observe the chat surface so history loaded or
  // rendered outside this module cannot leave the usage ring stale at 0%.
  const observerCtor = body.ownerDocument?.defaultView?.MutationObserver;
  let usageObserver: MutationObserver | null = null;
  if (chatBox && observerCtor) {
    try {
      const observer = new observerCtor(() => updateContextUsageIndicator());
      ...
      // A missing/unsupported observer must never abort handler setup. The
      // explicit refresh calls below remain sufficient for normal requests.
      ztoolkit.log("LLM: Context usage observer unavailable", err);
      usageObserver = null;
    }
  }
  ...
  const updatePaperPreview = () => {
    ...
    if (hasBasePdf && pool) {
      ...
      const baseSourceLabel = "Paper 1";
      const labelText = isCompressed
        ? `📝 ${baseSourceLabel} · ${basePdfTitle} [Summary]`
        : `📝 ${baseSourceLabel} · ${basePdfTitle}`;
    ...
    paperPreview
      ...
      .forEach((entry: Element) => entry.remove());
    if (clearPapersBtn) {
      clearPapersBtn.style.display = selectedPapers.length
        ? "inline-flex"
        : "none";
      clearPapersBtn.disabled = !selectedPapers.length;
    }
  ...
  const updateLiveSettingsReadinessState = (event: Event) => {
    ...
    const primaryConnectionMode = "custom" as const;
  ...
  const readSettingsDraftReadinessState =
    (): LiveSettingsReadinessState | null => {
      ...
      if (!customModeRadio.checked) {
        return { primaryConnectionMode: "custom" };
      }
  ...
  const getChatReadinessDetail = (): string => {
    ...
    const connectionMode =
      liveMode === "custom" ? liveMode : getPrimaryConnectionMode();
  ...
  const DRAFT_PREF_PREFIX =
    "extensions.zotero.zoteroResearchCopilot.draftInput.";
  ...
  const COMPOSE_PREF_PREFIX =
    "extensions.zotero.zoteroResearchCopilot.composeState.";
  ...
  const { doSend } = createSendFlowController({
    ...
    setStatusMessage: status
      ...
      : undefined,
    onContextEstimate: (tokens) => {
      const windowTokens = getContextWindowTokens();
      const percent = Math.min(100, Math.max(0, (tokens / windowTokens) * 100));
      const usedLabel =
      ...
      const windowLabel =
        windowTokens >= 1000000
          ? `${(windowTokens / 1000000).toFixed(1)}M`
          : `${Math.round(windowTokens / 1000)}K`;
      const label = `Context estimate: ${usedLabel} / ${windowLabel} tokens (${percent.toFixed(1)}%).`;
      contextUsage?.setAttribute("title", label);
      contextUsage?.setAttribute("aria-label", label);
      contextUsageRing?.style.setProperty("--llm-context-usage", `${percent}`);
    },
  ...
  if (slashCollectionOption) {
    slashCollectionOption.addEventListener("click", (e: Event) => {
      e.preventDefault();
      e.stopPropagation();
      closeSlashMenu();
      const libraryID = getCurrentLibraryID();
      if (!libraryID) {
        if (status) setStatus(status, "No active Zotero library", "error");
        return;
      }
      const collections = getLibraryCollectionOptions(libraryID);
      if (!collections.length) {
        if (status) setStatus(status, "No Zotero collections found", "warning");
        return;
      }
      const ownerDoc = body.ownerDocument;
      if (!ownerDoc) return;
      const overlay = createElement(
        ownerDoc,
        "div",
        "llm-collection-picker-overlay",
      );
      const dialog = createElement(
        ownerDoc,
        "div",
        "llm-collection-picker-dialog",
      );
      const title = createElement(
        ownerDoc,
        "strong",
        "llm-collection-picker-title",
        {
          textContent: "Add papers from Collection",
        },
      );
      const hint = createElement(
        ownerDoc,
        "div",
        "llm-collection-picker-hint",
        {
          textContent: "Choose a Collection. Up to 20 papers will be added.",
        },
      );
      const selectedPaperKeys = new Set<string>();
      const selectedPaperRefs = new Map<string, PaperContextRef>();
      const selectedStatus = createElement(
        ownerDoc,
        "div",
        "llm-collection-picker-selected",
        {
          textContent: `Selected: 0/${MAX_SELECTED_PAPER_CONTEXTS}`,
        },
      );
      const updateSelectedStatus = () => {
        selectedStatus.textContent = `Selected: ${selectedPaperKeys.size}/${MAX_SELECTED_PAPER_CONTEXTS}`;
      };
      const collectionList = createElement(
        ownerDoc,
        "div",
        "llm-collection-picker-list",
      );
      ...
      for (const collection of collections) {
        const group = createElement(
          ownerDoc,
          "div",
          "llm-collection-picker-group",
        );
        const option = createElement(
          ownerDoc,
          "button",
          "llm-collection-picker-option",
          {
            type: "button",
            textContent: `${collection.name} ▼`,
          },
        ) as HTMLButtonElement;
        const paperList = createElement(
          ownerDoc,
          "div",
          "llm-collection-picker-paper-list",
        );
        paperList.style.display = "none";
        let papersLoaded = false;
        option.addEventListener("click", (event) => {
          event.preventDefault();
          const open = paperList.style.display === "none";
          paperList.style.display = open ? "grid" : "none";
          option.textContent = `${collection.name} ${open ? "▲" : "▼"}`;
          if (!open || papersLoaded) return;
          papersLoaded = true;
          for (const entry of getCollectionPaperEntries(collection.id)) {
            const key = `${entry.ref.itemId}:${entry.ref.contextItemId}`;
            const paper = createElement(
              ownerDoc,
              "button",
              "llm-collection-picker-paper",
              {
                type: "button",
                textContent: `${entry.ref.title} · ${entry.hasPdf ? "PDF" : "Metadata only"}`,
              },
            ) as HTMLButtonElement;
            paper.setAttribute(
              "aria-pressed",
              selectedPaperKeys.has(key) ? "true" : "false",
            );
            paper.classList.toggle("selected", selectedPaperKeys.has(key));
            paper.addEventListener("click", (paperEvent) => {
              paperEvent.preventDefault();
              if (selectedPaperKeys.has(key)) {
                selectedPaperKeys.delete(key);
                selectedPaperRefs.delete(key);
                paper.classList.remove("selected");
                paper.setAttribute("aria-pressed", "false");
              } else {
                if (selectedPaperKeys.size >= MAX_SELECTED_PAPER_CONTEXTS) {
                  if (status)
                    setStatus(
                      status,
                      `Maximum ${MAX_SELECTED_PAPER_CONTEXTS} papers`,
                      "warning",
                    );
                  return;
                }
                selectedPaperKeys.add(key);
                selectedPaperRefs.set(key, entry.ref);
                paper.classList.add("selected");
                paper.setAttribute("aria-pressed", "true");
              }
        ...
        group.append(option, paperList);
        collectionList.appendChild(group);
      }
      const actions = createElement(
        ownerDoc,
        "div",
        "llm-collection-picker-actions",
      );
      const cancel = createElement(
        ownerDoc,
        "button",
        "llm-collection-picker-cancel",
        {
          textContent: "Cancel",
          type: "button",
        },
      ...
      const add = createElement(
        ownerDoc,
        "button",
        "llm-collection-picker-add",
        {
          textContent: "Add selected papers",
          type: "button",
        },
      ) as HTMLButtonElement;
      actions.append(cancel, add);
      dialog.append(title, hint, selectedStatus, collectionList, actions);
      ...
      add.addEventListener("click", (event) => {
        event.preventDefault();
        const refs = [...selectedPaperRefs.values()];
        if (!refs.length) {
          if (status)
            setStatus(status, "Select one or more papers first", "warning");
          close();
          return;
        }
        let added = 0;
        ...
        if (status) {
          setStatus(
            status,
            `Added ${added} selected paper${added === 1 ? "" : "s"}`,
            "ready",
          );
        }
        close();
  ...
  if (clearPapersBtn) {
    clearPapersBtn.addEventListener("click", (event: Event) => {
      event.preventDefault();
      event.stopPropagation();
      if (!item) return;
      clearSelectedPaperState(item.id);
      updatePaperPreview();
      if (status) setStatus(status, "Research papers cleared", "ready");
    });
  }
